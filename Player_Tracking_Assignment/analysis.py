...
def analyze_player_trajectories(results: dict):
    ...
    for i, (player_id, player_data) in enumerate(players.items()):
        ...
        if len(trajectory) > 1:
            ...
            ax.scatter(x_coords[0], y_coords[0], color=colors[i], 
                      s=100, marker='o', edgecolors='black', label=f'Start {player_id}')
            ax.scatter(x_coords[-1], y_coords[-1], color=colors[i], 
                      s=100, marker='s', edgecolors='black', label=f'End {player_id}')
    
    ax.set_title('Player Trajectories Analysis', fontsize=14)
    ...
    for player_id, player_data in players.items():
        ...
        print(f"Player {player_id}: {trajectory_length} trajectory points, "
              f"first seen at frame {first_seen}")
